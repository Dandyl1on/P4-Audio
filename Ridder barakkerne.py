...
def image_to_audio(sr, contrast_scale, MaxMag, original_audio_length):
    image = Image.open('Magnitude changes')
...
def main():
    ...
    folder_path = 'Phase changes'

    # Specify the file path within the folder
    file_path = os.path.join(folder_path, 'ChangeName.wav')

    # Write the reconstructed audio signal to the file
    wavfile.write(file_path, sr, reconstructed_audio)
