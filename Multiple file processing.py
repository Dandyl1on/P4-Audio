...
def main():
    ...
    for file_name in os.listdir(input_folder):
        if file_name.endswith('.wav'):
            ...
            output_image_path = os.path.join(output_folder, os.path.splitext(file_name)[0] + '_image.png')
            audio_to_image(magnitude, phase, contrast_scale, MaxMag).save(output_image_path)
            ...
            output_audio_path = os.path.join(output_folder, os.path.splitext(file_name)[0] + '_reconstructed.wav')
            wavfile.write(output_audio_path, sr, reconstructed_audio)
